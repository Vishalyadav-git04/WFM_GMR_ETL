...
Usage:
    python pipeline.py --pipeline mi       # Run MI pipeline only
    python pipeline.py --pipeline om       # Run O&M pipeline only
    python pipeline.py --pipeline all      # Run both pipelines
"""
...
from load.models import Base, ETLRunLog
from load.writer import init_tables, write_kpi
...
def _log_run(pipeline: str, status: str, error: str | None = None):
    from sqlalchemy.orm import Session
    from load.database import get_engine
    engine = get_engine()
    with Session(engine) as session:
        session.add(ETLRunLog(
            pipeline=pipeline,
            status=status,
...
def main():
    ...
    try:
        ...
        _log_run(args.pipeline, "failed", str(e))
        raise
